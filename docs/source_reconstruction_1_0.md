...
- **Name evidence is traceable, with a declared review debt.**
  ...
  stage-intro, 26 time-bonus, 26 message-render, and 26 message-script records
  were then reviewed, leaving 204 (`NAME-002`). Seven boss-identity records
  remain visual hypotheses (`NAME-003`); neither pending group is semantically
  confirmed.
...
| `NAME-002` | 204 records match the expanded 41-sentence generic detector; a further duplicate-basis review is pending. | partial |
