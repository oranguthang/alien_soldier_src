...
- **Name evidence is traceable, with a declared review debt.**
  ...
  then reviewed. The curated `NAME-002` detector has zero matches. Twenty of 240
  repeated-basis groups have exact-member reviews; 220 remain open. Seven
...
| `NAME-002` | Zero records match the curated 73-sentence generic detector; twenty repeated groups are accepted with exact members, and 220 remain open. | partial |
