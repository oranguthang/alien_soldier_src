...
- **Name evidence is traceable, with a declared review debt.**
  ...
  18 timer-return, 41 palette-fade, 66 OAM/mapping, and 30 object-pipeline
  records were then reviewed, leaving 302 (`NAME-002`). Seven boss-identity
  records remain visual hypotheses (`NAME-003`); neither pending group is
...
| `NAME-002` | 302 records match the expanded 41-sentence generic detector; a further duplicate-basis review is pending. | partial |
