...
- **Source, not blobs.** All executable 68000 code is assembly source across 383
...
- **Name evidence is traceable, with a declared review debt.**
  ...
  stage-intro, 26 time-bonus, 26 message-render, 26 message-script, and 20
  options-menu records were then reviewed, leaving 184 (`NAME-002`). Seven
  boss-identity records remain visual hypotheses (`NAME-003`); neither pending
  group is semantically confirmed.
...
| `NAME-002` | 184 records match the expanded 41-sentence generic detector; a further duplicate-basis review is pending. | partial |
