...
- `src/main.s` is an address-ordered index over 351 modules with a checked
  1,000-line ceiling; the current mean is 338.8 lines and no waiver is active.
- `config/rom_layout.json` is checked against listing addresses, landmarks,
  padding ranges, and the built image.
- Naming/evidence rules, 13,016 provenance mappings, and the remaining 3,032
...
3,032 address-derived definitions intentionally remain in the unknowns backlog.
