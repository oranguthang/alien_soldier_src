...
- `src/main.s` is an address-ordered index over 356 modules with a checked
  1,000-line ceiling; the current mean is 334.0 lines and no waiver is active.
- `config/rom_layout.json` is checked against listing addresses, landmarks,
  padding ranges, and the built image.
- Naming/evidence rules, 13,066 provenance mappings, and the remaining 2,982
...
2,982 address-derived definitions intentionally remain in the unknowns backlog.
