...
- `src/main.s` is an address-ordered index over 347 modules with a checked
  1,000-line ceiling; the current mean is 342.7 lines and no waiver is active.
- `config/rom_layout.json` is checked against listing addresses, landmarks,
  padding ranges, and the built image.
- Naming/evidence rules, 12,620 provenance mappings, and the remaining 3,427
...
3,427 address-derived definitions intentionally remain in the unknowns backlog.
