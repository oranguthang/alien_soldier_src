...
- `src/main.s` is an address-ordered index over 374 modules with a checked
  1,000-line ceiling; the current mean is 317.6 lines and no waiver is active.
- `config/rom_layout.json` is checked against listing addresses, landmarks,
  padding ranges, and the built image.
- Naming/evidence rules, 15,104 provenance mappings, and the remaining 947
...
The contract does not require invented semantics. The 947 address-derived
definitions in the current backlog comprise 831 RAM equates, 116 preserved-
data definitions, and no executable-source definitions. The exact-address
audit registry contains 12,275 records. A further 3,129 provenance-mapped
...
be folded into one record. It includes 445 reviewed asset `_End` aliases that
