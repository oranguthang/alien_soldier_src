...
- `src/main.s` is an address-ordered index over 377 modules with a checked
  1,000-line ceiling; the current mean is 315.1 lines and no waiver is active.
...
address audit registry contains 13,452 records. A further 2,891 provenance-mapped
...
upper bound is 2,378 names; no other pending `_End` labels are hidden by that
