...
- `src/main.s` is an address-ordered index over 368 modules with a checked
...
- Naming/evidence rules, 14,646 provenance mappings, and the remaining 1,405
...
The contract does not require invented semantics. The 1,405 address-derived
definitions in the current backlog comprise 831 RAM equates, 574 preserved-
data definitions, and no executable-source definitions. The exact-address
audit registry contains 11,969 records. A further 2,977 provenance-mapped
...
be folded into one record. It includes 286 player-art `_End` aliases that share
the following segment's address and do not represent separate semantic review
work. Promote or correct other names only with recorded static or runtime
evidence.
