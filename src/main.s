                cpu     68000
                page    0
                supmode on
                padding off

                include "src/macros.inc"
                include "src/ports.inc"
                include "src/equals.inc"
                include "src/ram_addrs.inc"

; ROM modules, strictly ordered by cartridge address
                include "src/system/cartridge_header.s"
                include "src/system/boot.s"
                include "src/system/region_check.s"
                include "src/gameplay/object_dispatch_table.s"
                include "src/system/vblank.s"
                include "src/rendering/vblank_dma.s"
                include "src/rendering/palette_fades.s"
                include "src/rendering/vblank_effects.s"
                include "src/rendering/hblank_effects.s"
                include "src/rendering/dma_queue.s"
                include "src/rendering/sprite_object_pipeline.s"
                include "src/rendering/sprite_mapping_and_oam.s"
                include "src/gameplay/object_data.s"
                include "src/rendering/tile_processing.s"
                include "src/system/game_initialization.s"
                include "src/system/memory_initialization.s"
                include "src/system/memory_clearing.s"
                include "src/system/input.s"
                include "src/sound/request_queue.s"
                include "src/math/angles.s"
                include "src/math/bcd_and_random.s"
                include "src/rendering/palette_transitions.s"
                include "src/rendering/color_fades.s"
                include "src/rendering/palette_effects.s"
                include "src/rendering/text_and_numbers.s"
                include "src/cutscenes/story_screen_and_title_transition.s"
                include "src/cutscenes/planet_ship_and_star_sequences.s"
                include "src/cutscenes/story_text.s"
                include "src/cutscenes/sprite_grid_and_pattern_effects.s"
                include "src/cutscenes/ending_sequence_credits.s"
                include "src/cutscenes/ending_starfield_and_planet.s"
                include "src/math/sine.s"
                include "src/cutscenes/ship_sequence.s"
                include "src/cutscenes/ship_piece_and_debris.s"
                include "src/ui/title_screen.s"
                include "src/ui/options_menu_controllers.s"
                include "src/ui/options_shared_helpers_and_assets.s"
                include "src/ui/password_screen.s"
                include "src/ui/message_sequence_engine.s"
                include "src/ui/stage_message_sequences.s"
                include "src/ui/message_scripts_and_glyph_lists.s"
                include "src/rendering/palettes.s"
                include "src/stages/early_stage_process_states.s"
                include "src/stages/stage7_transition_and_stage8_train_flying_neo.s"
                include "src/stages/stage_9_flies_caterpillar_and_xi_tiger.s"
                include "src/stages/terobuster_intro_projectiles.s"
                include "src/stages/flying_neo_effects.s"
                include "src/stages/stage10_to_stage13.s"
                include "src/stages/stage14_to_stage16.s"
                include "src/stages/stage17_epsilon1_states.s"
                include "src/projectiles/shared_directional_volley_helpers.s"
                include "src/rendering/snake_background_scroll.s"
                include "src/stages/late_game_state_dispatch.s"
                include "src/stages/stage18_to_stage20.s"
                include "src/stages/seven_forces_stage_states.s"
                include "src/stages/seven_forces_transition_graphics.s"
                include "src/stages/stage_and_boss_transition_states.s"
                include "src/stages/missiray_stage24_z_leo_transitions.s"
                include "src/stages/asteroid_and_destroyer_proto_scroll.s"
                include "src/stages/stage_transition_backdrop_effects.s"
                include "src/stages/wolf_garopa_arena_boundaries.s"
                include "src/stages/stage_process_dispatch.s"
                include "src/rendering/camera_tracking_and_stage_scroll.s"
                include "src/stages/stage_phase_transition_control.s"
                include "src/rendering/scroll_plane_buffers.s"
                include "src/rendering/tilemap_column_streaming.s"
                include "src/rendering/tilemap_row_streaming.s"
                include "src/rendering/tilemap_dma_primitives.s"
                include "src/rendering/vdp_asset_transfers.s"
                include "src/rendering/boss_asset_sets.s"
                include "src/stages/object_loading.s"
                include "src/stages/phase_loading.s"
                include "src/stages/visual_asset_loading.s"
                include "src/stages/configuration.s"
                include "src/rendering/stage3_tile_resampling.s"
                include "src/stages/configuration_records.s"
                include "src/ui/gameplay_hud.s"
                include "src/ui/hud_rendering.s"
                include "src/ui/hud_sprite_list.s"
                include "src/ui/weapon_state_icon_transfer.s"
                include "src/ui/debug_status_display.s"
                include "src/ui/debug_menu.s"
                include "src/collision/detection.s"
                include "src/collision/terrain_and_combat.s"
                include "src/player/terrain_collision.s"
                include "src/player/terrain_responses.s"
                include "src/player/core_states.s"
                include "src/player/dash_and_phoenix.s"
                include "src/player/fall_and_special_attack.s"
                include "src/player/cutscene_and_damage_states.s"
                include "src/player/air_and_ground_states.s"
                include "src/player/input_and_status.s"
                include "src/player/terrain_wrappers.s"
                include "src/player/rendering_and_defeat.s"
                include "src/player/projectiles_and_effects.s"
                include "src/ui/weapon_state_and_selection.s"
                include "src/weapons/firing.s"
                include "src/weapons/special_firing_and_feedback.s"
                include "src/weapons/projectile_impacts.s"
                include "src/system/object_pool_clearing.s"
                include "src/weapons/targeting_and_projectile_runtime.s"
                include "src/player/scripted_input_sequences.s"
                include "src/player/seven_forces_battle.s"
                include "src/gameplay/object_spawner.s"
                include "src/gameplay/math_and_buffer_helpers.s"
                include "src/gameplay/sprite_initialization.s"
                include "src/gameplay/projectile_slots.s"
                include "src/stages/gameplay_entry_states.s"
                include "src/gameplay/main_loop.s"
                include "src/debug/frame_timing_toggle.s"
                include "src/effects/plane_shake.s"
                include "src/debug/controller_helpers.s"
                include "src/gameplay/orphaned_object_record_setup.s"
                include "src/system/game_variables.s"
                include "src/ui/frontend_loop.s"
                include "src/ui/screen_transitions.s"
                include "src/cutscenes/scene_control.s"
                include "src/ui/post_stage_results_flow.s"
                include "src/ui/continue_screen.s"
                include "src/ui/results_screen.s"
                include "src/ui/stage_ready_and_retry_prompt.s"
                include "src/credits/entry.s"
                include "src/ui/stage_transition_messages.s"
                include "src/stages/transition_control.s"
                include "src/cutscenes/xi_tiger.s"
                include "src/stages/gameplay_initialization.s"
                include "src/ui/weapon_setup_screen.s"
                include "src/ui/weapon_setup_background_and_text.s"
                include "src/ui/results_scrolling.s"
                include "src/ui/results_data.s"
                include "src/credits/main.s"
                include "src/credits/palette_data.s"
                include "src/credits/animated_glyph_sequence.s"
                include "src/credits/z_leo_sequence.s"
                include "src/demo/playback.s"
                include "src/cutscenes/frame_projection.s"
                include "src/effects/wave_transition.s"
                include "src/effects/defeat_transition_control.s"
                include "src/effects/transition_scroll.s"
                include "src/cutscenes/game_over_landscape.s"
                include "src/effects/tunnel_transition.s"
                include "src/data/game_over_perspective_lookup.s"
                include "src/rendering/raster_buffer_layouts.s"
                include "src/projectiles/shared_boss_projectiles.s"
                include "src/actors/shared_object_helpers.s"
                include "src/debug/sprite_editor.s"
                include "src/data/shared_effect_sprite_frames.s"
                include "src/projectiles/directional_and_gravity_shots.s"
                include "src/projectiles/proximity_and_falling_hazards.s"
                include "src/actors/max_health_upgrade.s"
                include "src/ui/weapon_selection_object.s"
                include "src/actors/health_pickups.s"
                include "src/effects/explosion_patterns.s"
                include "src/projectiles/bugmax_debris_spawner.s"
                include "src/enemies/spawn_and_movement.s"
                include "src/enemies/jetsripper_stage_actors.s"
                include "src/enemies/projectile_attack_states.s"
                include "src/enemies/shared_enemy_helpers.s"
                include "src/enemies/phase_and_debris_states.s"
                include "src/enemies/circling_enemies.s"
                include "src/enemies/stage_9_flies_and_viblack_shots.s"
                include "src/enemies/bird_enemy.s"
                include "src/enemies/stage_10_wasp_and_falling_shot.s"
                include "src/enemies/stage_12_enemies.s"
                include "src/enemies/stage_10_beetles.s"
                include "src/enemies/stage_11_fish.s"
                include "src/cutscenes/xi_tiger_entrance_sequences.s"
                include "src/debug/orphaned_radial_particle_test.s"
                include "src/effects/stage_environment.s"
                include "src/stages/stage_12_yacht.s"
                include "src/debug/orphaned_cross_stage_handlers.s"
                include "src/enemies/stage_18.s"
                include "src/stages/stage_15_fragment_hazards.s"
                include "src/stages/stage_11_rising_hazards.s"
                include "src/enemies/entity_type_390_linked_chain.s"
                include "src/bosses/destroyer_proto_core.s"
                include "src/bosses/destroyer_proto_combat.s"
                include "src/bosses/destroyer_proto_defeat.s"
                include "src/projectiles/destroyer_proto_and_victor_projectiles.s"
                include "src/bosses/victor_core_and_ring.s"
                include "src/bosses/victor_parts_and_projectiles.s"
                include "src/bosses/wolf_garopa_reward_shower.s"
                include "src/enemies/formation_wave.s"
                include "src/stages/stage_21_asteroids.s"
                include "src/effects/rising_shot_waves.s"
                include "src/debug/orphaned_rising_shot_pair.s"
                include "src/projectiles/missiray_and_rising_shots.s"
                include "src/bosses/stage_3_orbiting_formation.s"
                include "src/rendering/boss_metasprites.s"
                include "src/data/antroid_terobuster_shellshogun_xi_tiger_metasprites.s"
                include "src/data/madam_barbar_flying_neo_joker_back_stringer_sharpssteel_metasprites.s"
                include "src/data/wolf_garopa_valkirie_z_leo_metasprites.s"
                include "src/bosses/jetsripper_core.s"
                include "src/bosses/jetsripper_segments.s"
                include "src/projectiles/jetsripper.s"
                include "src/bosses/sniper_honeyviper_core.s"
                include "src/bosses/sniper_honeyviper_movement.s"
                include "src/bosses/sniper_honeyviper_scroll_and_effects.s"
                include "src/projectiles/shellshogun_debris.s"
                include "src/projectiles/sniper_honeyviper.s"
                include "src/bosses/antroid_core.s"
                include "src/bosses/antroid_jump_attacks.s"
                include "src/bosses/antroid_charge_and_defeat.s"
                include "src/bosses/antroid_rendering.s"
                include "src/projectiles/antroid.s"
                include "src/bosses/antroid_animation.s"
                include "src/bosses/terobuster_core.s"
                include "src/bosses/terobuster_attacks.s"
                include "src/projectiles/terobuster.s"
                include "src/effects/terobuster_defeat_parts.s"
                include "src/bosses/terobuster_animation.s"
                include "src/bosses/shellshogun_core.s"
                include "src/bosses/shellshogun_rendering.s"
                include "src/bosses/madam_barbar_palette_cycle.s"
                include "src/bosses/shellshogun_animation.s"
                include "src/bosses/madam_barbar_core.s"
                include "src/projectiles/madam_barbar.s"
                include "src/bosses/joker_core.s"
                include "src/bosses/joker_rendering.s"
                include "src/bosses/flying_neo_core.s"
                include "src/bosses/flying_neo_rendering.s"
                include "src/bosses/flying_neo_parts_and_pose.s"
                include "src/bosses/caterpillar.s"
                include "src/bosses/xi_tiger_battle_states.s"
                include "src/bosses/xi_tiger_defeat_and_rendering.s"
                include "src/bosses/xi_tiger_pose_animation.s"
                include "src/bosses/deep_strider_controller_and_battle.s"
                include "src/bosses/deep_strider_defeat_and_dive_helpers.s"
                include "src/bosses/deep_strider_parts_and_projectiles.s"
                include "src/enemies/bouncing_object.s"
                include "src/bosses/gusthead_core.s"
                include "src/bosses/gusthead_combat.s"
                include "src/bosses/gusthead_tentacles.s"
                include "src/bosses/snake.s"
                include "src/bosses/entity_type_1c0_core.s"
                include "src/bosses/entity_type_1c0_attacks.s"
                include "src/bosses/entity_type_1c0_transition_and_defeat.s"
                include "src/math/shared_boss_angle_helpers.s"
                include "src/bosses/sunset_sting_wave.s"
                include "src/bosses/sunset_sting_main.s"
                include "src/bosses/sunset_sting_segments.s"
                include "src/bosses/viblack_core.s"
                include "src/bosses/viblack_support.s"
                include "src/projectiles/viblack_chain.s"
                include "src/bosses/back_stringer_core.s"
                include "src/bosses/back_stringer_support.s"
                include "src/projectiles/back_stringer_projectiles.s"
                include "src/bosses/epsilon_1_core.s"
                include "src/bosses/epsilon_1_attacks.s"
                include "src/bosses/epsilon_1_defeat.s"
                include "src/bosses/epsilon_1_ring_controller.s"
                include "src/projectiles/epsilon_1_projectiles.s"
                include "src/bosses/epsilon_1_shared_support.s"
                include "src/bosses/sharpssteel_core.s"
                include "src/bosses/sharpssteel_blades.s"
                include "src/projectiles/sharpssteel.s"
                include "src/bosses/jampan_core.s"
                include "src/bosses/jampan_attacks.s"
                include "src/bosses/jampan_support.s"
                include "src/bosses/jampan_geometry_and_input.s"
                include "src/bosses/destroyer_mk2_core.s"
                include "src/bosses/destroyer_mk2_patterns_and_components.s"
                include "src/bosses/destroyer_mk2_linked_parts_and_debris.s"
                include "src/bosses/bugmax_controller_and_geometry.s"
                include "src/bosses/bugmax_opening_states.s"
                include "src/bosses/bugmax_battle_and_final_sequence.s"
                include "src/projectiles/bugmax.s"
                include "src/bosses/bugmax_movement.s"
                include "src/bosses/shield_viper_core.s"
                include "src/bosses/shield_viper_attacks.s"
                include "src/projectiles/shield_viper.s"
                include "src/bosses/shield_viper_defeat.s"
                include "src/bosses/shield_viper_debug_and_effects.s"
                include "src/bosses/wolf_garopa_core.s"
                include "src/projectiles/wolf_garopa.s"
                include "src/bosses/wolf_garopa_defeat.s"
                include "src/bosses/wolf_garopa_attack_effects_and_transition.s"
                include "src/debug/valkirie_composite_viewer.s"
                include "src/debug/valkirie_secondary_composite_viewer.s"
                include "src/debug/valkirie_tertiary_composite_viewer.s"
                include "src/bosses/z_leo_core.s"
                include "src/bosses/z_leo_rendering.s"
                include "src/projectiles/z_leo.s"
                include "src/bosses/valkirie_force.s"
                include "src/bosses/missiray_core.s"
                include "src/bosses/missiray_attacks.s"
                include "src/bosses/missiray_segments.s"
                include "src/actors/unreferenced_nine_auxiliary_sprites.s"
                include "src/stages/stage_24_scene_object.s"
                include "src/cutscenes/seven_forces_intro_setup_and_debug.s"
                include "src/cutscenes/seven_forces_form_sequences.s"
                include "src/cutscenes/seven_forces_intro_finale_and_effects.s"
                include "src/cutscenes/seven_forces_post_battle_transitions.s"
                include "src/bosses/valkirie_battle.s"
                include "src/bosses/valkirie_rendering.s"
                include "src/bosses/medusa.s"
                include "src/bosses/sirene.s"
                include "src/bosses/artemis_core.s"
                include "src/bosses/artemis_rendering.s"
                include "src/projectiles/artemis.s"
                include "src/bosses/unidentified_seven_force.s"
                include "src/bosses/valkirie_alternate.s"
                include "src/bosses/sylpheed_core.s"
                include "src/bosses/sylpheed_pose.s"
                include "src/rendering/seven_forces_metasprites.s"
                include "src/data/pre_sound_padding.s"
                include "src/sound/driver_core.s"
                include "src/sound/command_dispatch_and_dac.s"
                include "src/sound/playback_and_loading.s"
                include "src/sound/hardware_interface.s"
                include "src/sound/volume_transitions.s"
                include "src/sound/sequence_dispatch_and_instrument_commands.s"
                include "src/sound/sequence_channel_and_extended_commands.s"
                include "src/sound/z80_program.s"
                include "src/sound/psg_playback_and_envelopes.s"
                include "src/sound/request_and_track_tables.s"
                include "src/sound/music_tracks.s"
                include "src/sound/sfx_tracks_a0_to_ef.s"
                include "src/sound/sfx_tracks_40_to_7f_and_f0_to_fc.s"
                include "src/sound/pcm_samples.s"
                include "src/data/sega_screen_assets.s"
                include "src/data/player_movement_and_dash_sprite_mappings.s"
                include "src/data/player_state_animation_sprite_mappings.s"
                include "src/data/player_special_and_death_sprite_mappings.s"
                include "src/data/shared_combat_sprite_frames.s"
                include "src/data/shared_combat_sprite_animations.s"
                include "src/data/frontend_sprite_mappings.s"
                include "src/data/weapon_select_and_enemy_sprite_mappings.s"
                include "src/data/enemy_projectile_animation_mappings.s"
                include "src/data/bird_animation_mappings.s"
                include "src/data/phase_pattern_sprite_mappings.s"
                include "src/data/stage10_wasp_sprite_mappings.s"
                include "src/data/circling_enemy_sprite_mappings.s"
                include "src/data/stage_combat_object_sprite_mappings.s"
                include "src/data/caterpillar_sprite_mappings.s"
                include "src/data/jetsripper_sprite_mappings.s"
                include "src/data/antroid_sprite_mappings.s"
                include "src/data/terobuster_sprite_mappings.s"
                include "src/data/shellshogun_sprite_mappings.s"
                include "src/data/sniper_honeyviper_tentacle_sprite_mappings.s"
                include "src/data/xi_tiger_sprite_mappings.s"
                include "src/data/madam_barbar_sprite_mappings.s"
                include "src/data/flying_neo_sprite_mappings.s"
                include "src/data/joker_sprite_mappings.s"
                include "src/data/xi_tiger_entrance_sprite_mappings.s"
                include "src/data/entity_type_1c0_part_sprite_mappings.s"
                include "src/data/deep_strider_sprite_mappings.s"
                include "src/data/entity_type_1c0_second_form_sprite_mapping.s"
                include "src/data/victor_and_sunset_sting_shared_sprite_mappings.s"
                include "src/data/sunset_sting_segment_sprite_mappings.s"
                include "src/data/viblack_missile_sprite_animations.s"
                include "src/data/gusthead_sprite_mappings.s"
                include "src/data/epsilon_1_sprite_mappings.s"
                include "src/data/sharpssteel_sprite_mappings.s"
                include "src/data/jampan_sprite_mappings.s"
                include "src/data/destroyer_mk2_sprite_mappings.s"
                include "src/data/back_stringer_sprite_mappings.s"
                include "src/data/unreferenced_post_back_stringer_sprite_mappings.s"
                include "src/data/valkirie_shared_rotation_c_mappings.s"
                include "src/data/valkirie_sprite_mappings.s"
                include "src/data/bugmax_sprite_mappings.s"
                include "src/data/seven_forces_valkirie_and_stage24_mappings.s"
                include "src/data/destroyer_proto_sprite_mappings.s"
                include "src/data/shield_viper_and_missiray_mappings.s"
                include "src/data/wolf_garopa_and_z_leo_mappings.s"
                include "src/data/shared_frontend_and_gameplay_tile_art.s"
                include "src/data/player_sprite_art.s"
                include "src/data/gameplay_tile_art.s"
                include "src/data/xi_tiger_and_boss_art.s"
                include "src/data/shared_boss_and_stage_mapping_data.s"
                include "src/data/japanese_font_and_credits_tile_art.s"
                include "src/data/credits_scene_assets.s"
                include "src/data/frontend_scene_assets.s"
                include "src/data/planet_cutscene_assets.s"
                include "src/data/shared_frontend_and_transition_assets.s"
                include "src/data/stage_object_assets.s"
                include "src/data/indexed_object_and_stage_effect_mappings.s"
                include "src/data/stage10_stage12_and_teleport_assets.s"
                include "src/data/shared_stage_object_sprite_mappings.s"
                include "src/data/stage10_and_teleport_mapping_data7800.s"
                include "src/data/stage10_and_teleport_tile_art_and_mapping_data.s"
                include "src/data/stage16_assets.s"
                include "src/data/stage2_phase1_assets.s"
                include "src/data/epsilon_1_mapping_data.s"
                include "src/data/ship_sequence_sprite_mappings.s"
                include "src/data/stage18_and_stage19_assets.s"
                include "src/data/stage2_phase2_assets.s"
                include "src/data/shared_stage18_and_stage20_mapping_data.s"
                include "src/data/stage20_assets.s"
                include "src/data/seven_forces_artemis_and_victory_assets.s"
                include "src/data/stage3_assets.s"
                include "src/data/stage24_scene_sprite_mapping.s"
                include "src/data/stage21_tile_art.s"
                include "src/data/stage_21_asteroid_and_destroyer_proto_mappings.s"
                include "src/data/stage_3_phase_3_tiles.s"
                include "src/data/missiray_entry_sprite_mappings.s"
                include "src/data/stage3_phase_mapping_data.s"
                include "src/data/rom_final_padding.s"
